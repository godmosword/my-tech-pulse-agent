...
These tests use OpenAI to evaluate the quality of pipeline outputs.
They require OPENAI_API_KEY and are intended to run in CI after the smoke tests pass.
...
from llm.json_client import generate_json, make_client

pytestmark = pytest.mark.skipif(
    not os.getenv("OPENAI_API_KEY"),
    reason="OPENAI_API_KEY not set — skipping LLM-as-judge tests",
)
...
def _judge(prompt: str, retries: int = 3) -> dict:
    ...
    raise last_exc or RuntimeError("judge failed")
