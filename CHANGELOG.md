...
### Added
- **財報多季趨勢圖（dashboard）**：`/earnings/report/[reportId]` 新增「多季趨勢」區，把已算好的 `EarningsTrend`（營收/EPS 等逐季點＋YoY/QoQ＋擴張/收縮方向）以 recharts 折線呈現（`components/earnings/EarningsTrendChart.tsx`，語義色、深淺色相容、稀疏/空資料 graceful、`aria-label`）。資料契約 detail-only：`getEarningsReport` 新增安全解析 `trend`（`parseEarningsTrend`＋`QuarterPointRow`/`MetricTrendRow`/`EarningsTrendRow`），**列表與 `/api/v1/earnings` payload 不變**；圖表經 `next/dynamic` 動態載入，不灌爆 report 路由初始 bundle。測試 `lib/earnings-trend.test.ts`、`EarningsTrendChart.test.tsx`。
- **持倉標記露出（dashboard）**：首頁「今日財報」每筆加「持倉／觀察」badge（共用 `components/data/PortfolioTierBadge.tsx`，由 `withPortfolioTierOnReports` 分類；`SignalsTable` 重構共用同一 badge）。
- **前端版面與欄位更直覺好用（dashboard）**：`NavRail`/`MobileMasthead` 改「閱讀／投資／系統」三組，左欄露出 5 個投資相關頁（持倉/訊號/財報/宏觀/校驗，文案避免暗示 URL 階層），手機維持 Today/Archive/Invest 三主入口＋次要系統列，不再一次塞 7–8 連結。Invest 中樞由 9 段平鋪改為兩層：「決策摘要」（今天重要的事＋部位脈動＋催化劑）置頂，下接 `InvestHubNav`（client，sticky `#anchor` 導覽＋IntersectionObserver active 高亮；非互斥 tab，保住整頁掃讀/搜尋/返回）與「深入分析」各區（加 `id`＋`scroll-mt`）。重疊的「與我持倉相關的新聞」降權為 `<details>` 收合並導回 Today（不刪除、1-click 可達）。新增 `components/MetricHint.tsx`＋集中 `lib/metric-glossary.ts`：IC／命中率／分位價差／posture／conviction 都有白話＋高低好壞，**非 hover-only**（click/tap/Enter 切換 popover、`aria-expanded`/`aria-describedby`、Escape/外點關閉），接入 `StatCard` 可選 `hint`。決策列抽共用 `components/data/MaterialMoveRow.tsx`＋`lib/material-move.ts` view-model（brief `BriefItem` 與 live `RenderableItem+portfolio_impact` 各自 map 成同一 `MaterialMoveView`，反證/下次檢查/市場狀態收進 `<details>` 降低密度）。文案中性化（`Invest Hub`→投資中樞、「Top 買進/迴避」→「分數較高/較低」）。`DataTable` 手機卡片補長值換行（`min-w-0 break-words`＋標籤 `shrink-0`）。純前端表現層、未改 API/型別/route/Firestore；`tests/material-move.test.ts`、`MetricHint.test.tsx` 補測（vitest 106→112）。
- **投資 artifacts 每日刷新（排程）**：`.github/workflows/refresh-invest-artifacts.yml` 在每日 pipeline 後（`00:00 UTC` = 08:00 台北）＋手動 `workflow_dispatch` 重算 `track_record.json`／`invest_brief.json`（`grade_decisions.py` best-effort + `build_invest_brief.py`，用既有 WIF 讀 Firestore），僅在有變更時 commit 回 `main`，讓 Vercel 端 dashboard 取得新資料。由 repo var `INVEST_ARTIFACTS_ENABLED` 控制（**現已於 prod 啟用 = `true`**，手動 `workflow_dispatch` 不受限）；`ci.yml` 加 `paths-ignore: backtest/results/**` 避免純資料 commit 觸發 Cloud Run 重建。CI 韌性修正：commit 步驟逐檔 stage（`grade_decisions` 在無 `FINNHUB_API_KEY` 時 best-effort 退出不阻擋 brief commit）、明確帶入 `FIRESTORE_PROJECT_ID`／`GOOGLE_CLOUD_PROJECT`／`FIRESTORE_COLLECTION_PREFIX`（WIF 憑證不帶預設 project，否則讀不到 material items）。runbook 見 `docs/SCHEDULED_RUNS.md`。
...
- **部位感知決策層（投資升級計劃 P1–P4）**：把每日新聞從「描述」轉成「對你帳本的決策」。
  ...
  - **P3 催化劑與 thesis**：`sources/catalyst_calendar.py` + `config/catalysts.yaml`（人工事件＋可併入 earnings 日曆）；`config/portfolio.yaml` 每持倉 optional `thesis`／`watch`（`sources/portfolio.py` 讀取）；`scoring/thesis_tracker.py` **只做證據連結**（支持／反駁／未來催化劑），不自動判定兌現、不出單一分數。
  - **P4 決策簡報＋反過度交易護欄**：`scoring/posture.py`（posture `no_action`/`monitor`/`review`/`risk_up`，預設保守、evidence 只能軟化語氣、cooldown 抑制重複提醒、附反證條件與下次檢查日，**移除買賣語言**）；Dashboard Invest 新增首區「今天對你的部位重要的事」（`DecisionBriefSection` 依 impact 排序、posture 標籤、影響持倉與理由），`MemoryItemSchema`／`RenderableItem` 加 `portfolio_impact`。
  - 測試：`tests/test_portfolio_impact.py`、`test_decision_context.py`、`test_posture.py`、`test_catalyst_and_thesis.py`、`dashboard/lib/portfolio-brief.test.ts`。全 additive、不改既有訊號決策與對外契約。（後續：P2 live 價格/估值抓取與 P3 thesis 專頁待接線；posture 的 evidence/cooldown 全邏輯在 Python，dashboard 目前顯示 impact-band 標籤。）
